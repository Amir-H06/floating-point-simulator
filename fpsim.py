...
"""Introducing some sort of documentation - it's 10pm on a sunday night so 
...
def splitsignif(a, b, c):
    ...
    k = c.e - b.e
    h = c.e - a.e
    assert p == a.ctx.p == b.ctx.p  # checks all fp significand is the same

    # c1 is basically the same as floating point c, but we added 0s to the 
    # left to make the bit_length the same as p
    ...
    if checkcarry(fpb[1], fpc[0], p):  # generate
        if subtractcarry(fpb[1] + fpc[0], fpa[0], p, h):
            ...
            if (fpa[2] > fpb[2] or (fpb[2] == fpa[2] and fpa[3] > 0)):  # case 2.8
                if not eval(check(a, b, c, f8_rtz, 2, 8), ((not fpa[4]) or (fpb[3] ^ fpc[1])) and (fpa[5] > 0), 2, 8):
                    print(i, j, l, check(a, b, c, f8_rtz, 2, 8))
                    logging.debug('%s %s %s', i, j, l)
            else:
                eval(check(a, b, c, f8_rtz, 2, 7), (((fpa[5] == 0) and fpa[4] and (fpb[3] ^ fpc[1])) or ((fpa[5] > 0) and (fpa[4] or (fpb[3] ^ fpc[1])))), 2, 7)
        else:
